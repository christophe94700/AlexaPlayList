...
if($ip_ruleset != 'OFF'){
    ...
    if($proceed == false){
		echo lng('User connection denied from: ') . $clientIp;
...
if (isset($_GET['del']) && !FM_READONLY) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (fm_rdelete($path . '/' . $del)) {
            $msg = $is_dir ? (lng('Folder'). '<b> %s </b>'.lng( 'deleted')) : 'File <b>%s</b> deleted';
...
if (isset($_GET['ren'], $_GET['to']) && !FM_READONLY) {
    ...
    if (fm_isvalid_filename($new) && $old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            ...
            fm_set_msg(sprintf('Error while renaming from <b>%s</b> to <b>%s</b>', fm_enc($path . '/' . $old), fm_enc($path . '/' . $new)), 'error');
        }
    } else {
...
if (isset($_POST['listemp3'])&& isset($_POST['create'])) {
	...
	if(!empty($copy_files)){
		foreach ($copy_files as $file) {
			...
			$playlist.=str_replace(array("%2F", "%3A"),array("/", ":"),rawurlencode($file))."\n";
		}
	}
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <p><h3><a href="https://domotronic.fr" target="_blank" class="app-v-title"> Alexa List MP3 <?php echo VERSION; ?></a></h3></p>
                        <p><?php echo lng('Author') ?>: Christophe Caron</p>
        ...
        <div class="col-12">
			...
			?>
                ...
                <p>
                    ...
                    <b><a href="<?php echo $str.'/'.$file?>" target="_blank"><i class="fa fa-external-link-square"></i> <?php echo lng('Open') ?></a></b>
                    &nbsp;
                    <?php
                    // ZIP actions
            ...
            if($is_onlineViewer) {
                ...
                echo '<p><audio src="' .$str.'/'.$file. '" controls preload="metadata"></audio></p>';
            } elseif ($is_video) {
                // Video content
        ...
        <div class="col-xs-12 col-sm-9">
            <ul class="list-inline footer-action">
				...
				<li class="list-inline-item"><input type="submit" class="hidden" name="list" id="a-list" value="List">
                    <a href="javascript:document.getElementById('a-list').click();" class="btn btn-small btn-outline-primary btn-2"><i class="fa fa-th-list"></i> <?php echo lng('List MP3') ?> </a></li>
            </ul>
        </div>
        <div class="col-3 d-none d-sm-block"><a href="https://domotronic.fr" target="_blank" class="float-right text-muted">Alexa List MP3 <?php echo VERSION; ?></a></div>
        <?php else: ?>
		<div class="col-xs-12 col-sm-9">
            <ul class="list-inline footer-action">
                <li class="list-inline-item"> <a href="#/select-all" class="btn btn-small btn-outline-primary btn-2" onclick="select_all();return false;"><i class="fa fa-check-square"></i> <?php echo lng('SelectAll') ?> </a></li>
            ...
            <div class="col-12"><a href="https://domotronic.fr" target="_blank" class="float-right text-muted">Alexa List MP3 <?php echo VERSION; ?></a></div>
        <?php endif; ?>
    </div>
...
{
    //$isFileAllowed = fm_is_valid_ext($new); Bug to rename folder
...
<head>
    ...
    <meta name="author" content="Domotronic.fr">
...
<script>
    ...
    //TFM Config
    window.curi = <?php $dir_php;?>+ "/config.json", window.config = null;
